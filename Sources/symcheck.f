...
!-----------------------------------------------
      INTEGER                                     :: sym_nfp
      INTEGER                                     :: sym_nextcur
      REAL (rprec), ALLOCATABLE, DIMENSION(:,:,:) :: sym_br
      REAL (rprec), ALLOCATABLE, DIMENSION(:,:,:) :: sym_bz
      REAL (rprec), ALLOCATABLE, DIMENSION(:,:,:) :: sym_bp
      REAL (rprec), ALLOCATABLE, DIMENSION(:,:,:) :: sym_b
!-----------------------------------------------
...
!-----------------------------------------------
      ...
      INTEGER, PUBLIC                             :: sym_ir
      INTEGER, PUBLIC                             :: sym_jz
      INTEGER, PUBLIC                             :: sym_kp
...
!  Compute br, bp, bz at each grid point
     ...
     &                     sym_bp(i,j,k), sym_bz(i,j,k), ig)
                  
!  Compute magnitude, b, at each grid point
               sym_b(i,j,k) = DSQRT(sym_br(i,j,k)**2 +                         &
     &                              sym_bp(i,j,k)**2 +                         &
     &                              sym_bz(i,j,k)**2)
...
!-----------------------------------------------
      ...
      REAL (rprec), INTENT(out) :: maxerr
      REAL (rprec), INTENT(out) :: avgerr
!-----------------------------------------------
!   L o c a l   V a r i a b l e s
!-----------------------------------------------
      INTEGER                   :: i
...
!-----------------------------------------------
      ...
      DO k = ki + sym_kp, sym_kp*sym_nfp, sym_kp
         ...
         IF (currerr .GT. maxerr) THEN
            maxerr = currerr
         END IF
      END DO
      
      IF (sym_nfp .GT. 1) THEN
         avgerr = avgerr/(sym_nfp - 1)
      END IF
...
!-----------------------------------------------
     ...
     &                    + DSQRT(dbr**2 + dbp**2 + dbz**2)/avgb
      ...
      IF (midk-startk + 1 .GT. 0) THEN
         avgerr = avgerr/(midk - startk + 1)
      END IF
